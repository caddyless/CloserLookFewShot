...
if __name__ == '__main__':
    ...
    if params.stop_epoch == -1:
        if params.method in ['baseline', 'baseline++']:
            if params.dataset in ['omniglot', 'cross_char']:
                ...
                # This is different as stated in the open-review paper.
                # However, using 400 epoch in baseline actually lead to
                # over-fitting
    ...
    if params.method in ['baseline', 'baseline++']:
        ...
        val_loader = val_datamgr.get_data_loader(val_file, aug=False)
        # a batch for SetDataManager: a [n_way, n_support + n_query, dim, w, h]
        # tensor
        ...
        if params.method == 'protonet':
            ...
            if params.dataset in [
                'omniglot',
                    'cross_char']:  # maml use different parameter in omniglot
                model.n_task = 32
    ...
    if params.resume:
        ...
        if tmp is not None:
            ...
            for i, key in enumerate(state_keys):
                if "feature." in key:
                    # an architecture model has attribute 'feature', load
                    # architecture feature to backbone by casting name from
